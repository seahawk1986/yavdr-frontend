...
INTERFACE_NAME = "de.yavdr.frontend"


# <node>
#     <interface name='de.yavdr.frontend.Controller'>
class yaVDRFrontend(DbusInterfaceCommonAsync, interface_name=INTERFACE_NAME):
    ...
    @dbus_method_async(result_signature="bs", flags=sdbus.DbusUnprivilegedFlag)
    async def toggle_noninteractive(self):
        raise NotImplementedError
    ...
    async def switchto(self, next_frontend: str):
        raise NotImplementedError
    ...
    async def set_next(self, next_frontend: str):
        raise NotImplementedError
        #         <method name='setNext'>
        #             <arg type='s' name='frontend' direction='in'/>
        #             <arg type='b' name='response' direction='out'/>
    ...
    async def set_display(self, display: str):
        raise NotImplementedError
    ...
    async def set_next_fe(self, type: str, name: str, class_: str) -> bool:
        raise NotImplementedError
    ...
    def current_frontend(self) -> str:
        return self.current_frontend


async def export_frontend(config: Config):
    ...
    await interface_bus.request_name_async(INTERFACE_NAME, 0)

    print("export the Interface to the bus")

    interface = yaVDRFrontend()
    interface.export_to_dbus("/", interface_bus)

    async with asyncio.TaskGroup():
        lirc_connection = asyncio.create_task(  # noqa: F841
